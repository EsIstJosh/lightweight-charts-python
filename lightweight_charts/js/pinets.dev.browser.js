...
(function (global, factory) {
  ...
  class PineTS {
    ...
    async loadMarketData(source, tickerId, timeframe, limit, sDate, eDate, title) {
      if (Array.isArray(source)) {
        return source;
      } else {
        return source.getMarketData(tickerId, timeframe, limit, sDate, eDate, title);
      }
    }
    async ready() {
  ...
  class Core {
    ...
    plotcandle(open, high, low, close, title, options) {
      if (this.script) {
        options.group = this.script;
      }
    ...
    plotbar(open, high, low, close, title, options) {
      if (this.script) {
        options.group = this.script;
      }
  ...
  class TechnicalAnalysis {
    ...
    vwap(source, anchor, stdev_mult) {
      const result = vwapFunction(source, this.context.data.volume, anchor, stdev_mult);
      const idx = this.context.idx;
      if (stdev_mult !== void 0 && Array.isArray(result)) {
        const [v, up, low] = result;
        return [
  ...
  function vwapFunction(source, volume, anchor, stdev_mult) {
    ...
    let upper = [];
    let lower = [];
    const calculateBands = stdev_mult !== void 0;
    if (calculateBands) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (calculateBands) {
        ...
        const variance2 = segSumSq / segCount - segMean * segMean;
