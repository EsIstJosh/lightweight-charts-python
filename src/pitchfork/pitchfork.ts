...
export const defaultPitchForkOptions: PitchForkOptions = {
    ...
    variant: 'standard',
    forkLines: [
      { value: 1.0, width: 2, style: LineStyle.Solid, color: "#ff0000" ,fillColor: undefined},
      { value: 0.786, width: 1 as LineWidth, style: LineStyle.SparseDotted, color: "#000fff" ,fillColor: undefined},
      { value: .618, width: 1, style: LineStyle.LargeDashed, color: "#ffffff" ,fillColor: undefined},
      { value: .5, width: 2, style: LineStyle.Solid, color: "#ff0000" ,fillColor: undefined},
      { value: 0.382, width: 1, style: LineStyle.LargeDashed, color: "#ffffff" ,fillColor: undefined},
      { value: 0.236, width: 1 as LineWidth, style: LineStyle.SparseDotted, color: "#000fff" ,fillColor: undefined},
      { value: 0, width: 2, style: LineStyle.Solid, color: "#ff0000" ,fillColor: undefined}
    ],
...
export class PitchFork extends ThreePointDrawing {
    _type = "PitchFork"
    public readonly variant: 'standard' | 'schiff' | 'modifiedSchiff' | 'inside';
    constructor(
        p1: Point,
        p2: Point,
        p3: Point,
    ...
    _moveToState(state: InteractionState) {
     ...
     _onDrag(diff: any) {
        ...
        if (this._state == InteractionState.DRAGGING || this._state == InteractionState.DRAGGINGP3) {
    ...
    protected _onMouseDown() {
        ...
        if (!p1.x || !p2.x || !p3.x || !p1.y || !p2.y || !p3.y) return this._moveToState(InteractionState.DRAGGING);
        ...
        else if (Math.abs(hoverPoint.x-p3.x) < tolerance && Math.abs(hoverPoint.y-p3.y) < tolerance) {
    ...
    protected _mouseIsOverDrawing(param: MouseEventParams, tolerance = 4): boolean {
        ...
        if (mouseX < Math.min(x1, x2, x3) - tolerance || mouseX > Math.max(x1, x2, x3) + tolerance) {
          return false;
        }
      ...
      private _distanceFromSegment(xA: number, yA: number, xB: number, yB: number, x: number, y: number): number {
        const A = x - xA, B = y - yA, C = xB - xA, D = yB - yA;
        const dot = A * C + B * D;
        const lenSq = C * C + D * D;
        let param = (lenSq !== 0) ? dot / lenSq : -1;
        let xx: number, yy: number;
        if (param < 0) { xx = xA; yy = yA; }
        else if (param > 1) { xx = xB; yy = yB; }
        else { xx = xA + param * C; yy = yA + param * D; }
